...
function safeUser(u) {
  return { id: u.id, username: u.username, phone: u.phone, email: u.email, role: u.role, name: u.name, avatar: u.avatar };
}
...
router.post('/register', async (req, res) => {
  try {
    ...
    const user = await prisma.user.create({ data: { username, password: bcrypt.hashSync(password, 10), name, phone } });
    res.json({ id: user.id });
