  ...
  computeBoxSize(() => {
    nextTick(() => {
      raf(() => {
        ...
        if (boxSize.value > 0) {
          canvas2d.resize(boxSize.value, boxSize.value);
          drawBackground();
          if (paths.length) redrawCanvas(false);
          pointer.refreshRect();
          // #ifdef H5
...
// ── 初始化（Canvas 2D 同层渲染，H5 / 小程序同款 API）──
function initCanvas() {
  canvas2d.init();
}
...
function onTouchStart(e) {
  pointer.refreshRect(() => {
    ...
    // 单指：始终绘画
    lastPinchDist = null;
    handleDrawStart(pointer.fromTouch(touches[0]));
  });
}

function onTouchMove(e) {
  pointer.syncTransformedRectDom();
  const touches = e.touches;
  ...
  // 单指：绘画
  if (touches.length === 1) {
    handleDrawMove(pointer.fromTouch(touches[0]));
  }
}
...
// ── 鼠标事件（H5 原生 canvas 监听）──
function onMouseDown(e) {
  isMouseDown = true;
  pointer.refreshRect(() => {
    handleDrawStart(pointer.fromMouseEvent(e));
  });
}
...
function onMouseMove(e) {
  ...
  pointer.syncTransformedRectDom();
  handleDrawMove(pointer.fromMouseEvent(e));
}
...
.toolbar {
  ...
  padding: 12rpx 24rpx calc(32rpx + env(safe-area-inset-bottom, 0px));
}
