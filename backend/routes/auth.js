...
async function recordLogin(userId) {
  try {
    ...
    const exists  = await prisma.userLoginLog.findFirst({ where: { userId, dateStr } });
    if (!exists) {
      await prisma.userLoginLog.create({ data: { userId, dateStr } });
      checkAchievements(userId, 'login'); // 异步，不阻塞响应
    }
  } catch (e) {
    console.error('[recordLogin]', e.message);
  }
}
