...
defineOptions(createMpShare('homework/cbt'));
// #endif
...
const fields = [
  ...
  { key: 'autoThought',     label: '自动想法',         hint: '脑海中浮现了什么念头？',       required: true },
  { key: 'emotion',         label: '情绪及强度',       hint: '如：焦虑80%、悲伤60%',         required: true },
  { key: 'evidenceFor',     label: '支持该想法的证据', hint: '哪些事实支持这个想法？',       required: false },
  { key: 'evidenceAgainst', label: '不支持该想法的证据', hint: '哪些事实与这个想法矛盾？',  required: false },
  { key: 'balancedThought', label: '理性想法',         hint: '更平衡的看法是什么？',         required: false },
  { key: 'outcome',         label: '结果',             hint: '重新评估情绪，有什么变化？',   required: false },
];
...
async function save() {
  if (!form.value.situation || !form.value.autoThought || !form.value.emotion) {
    uni.showToast({ title: '请填写带*的必填项', icon: 'none' }); return;
  }
  try {
    const res = await homeworkApi.cbtCreate(form.value);
    track('homework_save', 'cbt');
    form.value = {};
    await load();
...
async function appendNote() {
  ...
  await homeworkApi.cbtUpdate(selected.value.id, { ...selected.value, notes });
...
function del(id) {
  uni.showModal({ title: '确认删除', content: '删除后不可恢复', success: async ({ confirm }) => {
    if (!confirm) return;
    try { await homeworkApi.cbtDel(id); await load(); } catch {}
  }});
}
