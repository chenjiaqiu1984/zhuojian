  ...
  <view class="home">
    ...
    >
      <view class="hero-content">
        ...
        <view class="hero-actions">
          <view class="hero-btn-primary" @click.stop="goIsland()">
        ...
        <view class="nav-grid">
          <view class="nav-item" v-for="item in group.items" :key="item.path" @click="nav(item.path)">
            <view class="nav-icon" :style="{background: item.color || '#4A8A7A'}">
              <image :src="iconSrc(item.icon)" class="nav-icon-img" mode="aspectFit" />
            </view>
    ...
    <view class="icp-footer">
      ...
      <view class="beian-row" @click="openBeian()">
        <image class="beian-icon" :src="staticUrl('/static/beian.png')" mode="aspectFit" />
...
import { resolveGridIcon } from '../../utils/gridIcons';
import IslandHero from '../../components/IslandHero.vue';

function iconSrc(icon) {
  // H5：优先 data URL，规避云端 svg MIME 错误；其他端走 staticUrl
  return resolveGridIcon(icon) || staticUrl(icon) || icon || '';
}
...
defineOptions({
  onShareAppMessage() {
    return { title: '卓见心理 — 功能导航', path: '/pages/home/index' };
  },
  onShareTimeline() {
